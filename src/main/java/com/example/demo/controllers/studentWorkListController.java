...
@Controller
public class studentWorkListController {
    @RequestMapping("studentWorkList")
    public String studentWorkList(ModelMap map, HttpSession session){
        List<WorkListBean> workList = new ArrayList<>();//任务列表
        String studentNo = session.getAttribute("userNo").toString();
        try {
            Connection conn = GetConnection.getConn();
            String getTeacherNo = "select teacherno from student , teacher \n" +
                    "where teacher.CLASSNO = student.CLASSNO and studentno = '"+studentNo+"'";
            PreparedStatement pre = conn.prepareStatement(getTeacherNo);
            ResultSet teacherNoResult = pre.executeQuery();
            String teacherNo = "";
            while(teacherNoResult.next()){
                teacherNo = teacherNoResult.getString("TEACHERNO");
            }
